...
getsession(): Observable<SessionCd[]> {
  ...
  search(idDirection : string | '' , statut: string | '', motif : string | '', datedebut :string | '', datefin :string | '', session : string | '' , idfournisseur : string | '' , idsession :string | ''): Observable<DetailDemande[]>{
    const headers = this.getHeaders();
